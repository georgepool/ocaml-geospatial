...
let () =
  ...
  let layer_total = UInt8OperationLayer.sum_layer (UInt8OperationLayer.SingleLayer layer) in
  ...
  Eio.traceln "The total of the operation: %i"
    (UInt8OperationLayer.sum_layer (UInt8OperationLayer.SingleLayer output_layer));
  Eio.traceln "----------------------------------";
  ...
  Eio.traceln "The total after adding 1 to everything: %i"
    (UInt8OperationLayer.sum_layer (UInt8OperationLayer.SingleLayer output_layer2));
  ...
  Eio.traceln "The total after adding 1 to empty layer: %i"
    (UInt8OperationLayer.sum_layer (UInt8OperationLayer.SingleLayer output_layer3));
...
(* 
  ...
  let intersection_area =
    UInt8Layer.find_intersection [ layer; empty_layer ]
  in
